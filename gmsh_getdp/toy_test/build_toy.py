...
# Cross-section the winding current crosses: the coil's r-z section
# (radial thickness x height). NOT the annulus pi*(ro^2-ri^2) -- current
# here is AZIMUTHAL, so it passes through a plane CONTAINING the axis.
# This was wrong until 2026-09-19 and the error was invisible, because the
# same bad area was used for BOTH Jmag and the analytic reference below:
# the two agreed to 1.4% at 10.19 mT while the true answer is 28.02 mT.
A_cross = (OD / 2 - ID / 2) * height
Jmag = turns * current / A_cross


def B_thick_solenoid_center(J, r_in, r_out, h):
    """Exact on-axis centre field of a thick finite solenoid:
    B = mu0*J*b*ln[(a2+sqrt(a2^2+b^2))/(a1+sqrt(a1^2+b^2))], b = h/2.
    Independent of the FEM -- this is what the solve is checked against."""
    b = h / 2
    return mu0 * J * b * math.log(
        (r_out + math.hypot(r_out, b)) / (r_in + math.hypot(r_in, b))
    )


B_ideal = B_thick_solenoid_center(Jmag, ID / 2, OD / 2, height)
# The infinite-solenoid estimate, for scale only -- this coil's wall is
# thick enough relative to its radius that it overpredicts substantially.
B_infinite = mu0 * turns * current / height

print(f"A_cross = {A_cross:.6e} m^2  (r-z section, not the annulus)")
print(f"Jmag    = {Jmag:.6e} A/m^2")
print(f"B_ideal (center, on-axis, exact thick solenoid) = {B_ideal*1e3:.4f} mT")
print(f"  [infinite-solenoid mu0*N*I/L, for scale only:  {B_infinite*1e3:.4f} mT]")
...
# Feed the source term to the solve rather than relying on a literal
# hand-copied into magnetostatics_toy.pro -- that copy went stale and kept
# the solve running the old current density after this script changed.
with open("toy_source_generated.pro", "w") as f:
    f.write("// AUTO-GENERATED by build_toy.py -- do not hand-edit.\n")
    f.write("Function {\n")
    f.write(f"  Jmag = {Jmag!r};  // A/m^2 = turns*current/A_cross (r-z section)\n")
    f.write("}\n")
